...
typedef enum
{
	EXPLAIN_RESULT_INVALID = 0,
	EXPLAIN_CONNECTION_FAILED = 1,
	EXPLAIN_QUERY_FAILED = 2,
	EXPLAIN_QUERY_SUCCEEDED = 3
} RemoteExplainResult;
...
static void ExplainTask(Task *task, ExplainState *es);
static RemoteExplainResult ExplainTaskPlacement(Task *task, int placementIndex,
												ExplainState *es);
					 ...
					 const char *queryString, ParamListInfo params)
{
	...
	if (list_length(workerJob->dependedJobList) > 0)
	{
		appendStringInfo(es->str, "explain statements for complex queries ");
...
ExplainJob(Job *job, ExplainState *es)
{
	...
	if (ExplainAllTasks || taskCount <= 1)
	{
		ExplainPropertyText("Tasks Shown", "All", es);
	}
	else
	{
		ExplainPropertyText("Tasks Shown", "Sample", es);
	}
...
/*
 * ExplainTask shows the EXPLAIN plan for a given task. The current
 * implementation tries each shard placement until one succeeds or
 * all failed.
 */
static void
ExplainTask(Task *task, ExplainState *es)
{
	List *taskPlacementList = task->taskPlacementList;
	int placementCount = list_length(taskPlacementList);
	int placementIndex = 0;
	RemoteExplainResult taskResult = EXPLAIN_RESULT_INVALID;
	...
	{
		RemoteExplainResult placementResult = EXPLAIN_RESULT_INVALID;

		placementResult = ExplainTaskPlacement(task, placementIndex, es);
		if (placementResult == EXPLAIN_QUERY_SUCCEEDED)
		{
			/* if a query succeeded, show no error message */
			taskResult = EXPLAIN_QUERY_SUCCEEDED;
			break;
		}
		else if (placementResult == EXPLAIN_QUERY_FAILED)
		{
			/* if at least one query failed, show the query error message */
			taskResult = EXPLAIN_QUERY_FAILED;
		}
		else if (taskResult == EXPLAIN_RESULT_INVALID)
		{
			/* if all connections failed, show the connection error message */
			taskResult = EXPLAIN_CONNECTION_FAILED;
		}
	}

	if (taskResult == EXPLAIN_QUERY_FAILED)
	{
		ExplainPropertyText("Error", "Could not get remote plan. You may need to "
									 "run the query first.", es);
	}
	else if (taskResult == EXPLAIN_CONNECTION_FAILED)
	{
		ExplainPropertyText("Error", "Could not connect to any placements.",
							es);
	}
...
/*
 * ExplainTaskPlacement performs a remote EXPLAIN of a task query on
 * a shard placement specified by placementIndex. Returns whether the
 * query succeeded, the connection failed, or the query returned an
 * error. The latter can happen when data fetches are necessary to
 * run the EXPLAIN.
 */
static RemoteExplainResult
ExplainTaskPlacement(Task *task, int placementIndex, ExplainState *es)
{
	...
	{
		return EXPLAIN_CONNECTION_FAILED;
	}

	querySent = MultiClientSendQuery(connectionId, explainQuery->data);
	if (!querySent)
	{
	...
	for (rowIndex = 0; rowIndex < rowCount; rowIndex++)
	{
		...
		StringInfo rowString = makeStringInfo();
		int rowLength = 0;
		char *lineStart = NULL;

		appendStringInfoString(rowString, rowValue);
	...
	return EXPLAIN_QUERY_SUCCEEDED;
}
